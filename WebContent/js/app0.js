...
app.factory('CmrService', [
    ...
    function($http, $q) {
      ...
      service.validate = function(request, arrOfValidators, addrMode, addrIndex) {
        var errCount = 0;
        var cmrIssuingCntry = document.getElementById('cmrIssuingCntry').value;
        var reqType = document.getElementById('reqType').value;
        for ( var prop in request) {
          if (request.hasOwnProperty(prop)) {

            if (addrMode && _ADDRESS_FIELDS.indexOf(prop) < 0) {
              continue;
            }
            if (!addrMode && _ADDRESS_FIELDS.indexOf(prop) >= 0) {
              continue;
            ...
            // DE Update request validate fix work around
            if (prop && prop == 'custNm1' && cmrIssuingCntry == '724' && reqType == 'U') {
              continue;
            }
            ...
            request[prop].error = false;
            var selector = addrMode ? '[name="' + prop + '"]' : '#' + prop;
            var errSelector = addrMode ? '[name="err_' + prop + '"]' : '#err_' + prop;

            if (addrMode) {
              if (jQuery(jQuery(selector)[addrIndex]).data() && jQuery(jQuery(selector)[addrIndex]).data().select2) {
                jQuery(jQuery(selector)[addrIndex]).data().select2.$selection.removeClass("ibm-field-error");
              }
            } else {
              if (jQuery(selector).data() && jQuery(selector).data().select2) {
                jQuery(selector).data().select2.$selection.removeClass("ibm-field-error");
              }
            }

            if (request[prop].required && !request[prop].value) {
              // check if the field is in the UI or not

              if (addrMode) {
                jQuery(jQuery(errSelector)[addrIndex]).html('This field is required.');
                if (jQuery(jQuery(selector)[addrIndex]).data() && jQuery(jQuery(selector)[addrIndex]).data().select2) {
                  jQuery(jQuery(selector)[addrIndex]).data().select2.$selection.addClass("ibm-field-error");
                }
              } else {
                jQuery(errSelector).html('This field is required.');
                if (jQuery(selector).data() && jQuery(selector).data().select2) {
                  jQuery(selector).data().select2.$selection.addClass("ibm-field-error");
                }
              }

              request[prop].error = true;
              console.log('error required ' + prop);
              errCount++;
            }

            if (arrOfValidators) {
              for (var i = 0; i < arrOfValidators.length; i++) {
                var validator = arrOfValidators[i];
                if (validator.getFieldName() != prop) {
                  continue;
                }
                if (!validator.validate(request[prop].value)) {
                  if (addrMode) {
                    jQuery(jQuery(errSelector)[addrIndex]).html(validator.getMsg());
                    if (jQuery(jQuery(selector)[addrIndex]).data() && jQuery(jQuery(selector)[addrIndex]).data().select2) {
                      jQuery(jQuery(selector)[addrIndex]).data().select2.$selection.addClass("ibm-field-error");
                    }
                  } else {
                    jQuery(errSelector).html(validator.getMsg());
                    if (jQuery(selector).data() && jQuery(selector).data().select2) {
                      jQuery(selector).data().select2.$selection.addClass("ibm-field-error");
                    }
                  }
                  request[prop].error = true;
                  console.log('error validator ' + prop);
                  errCount++;
        ...
        if (request.addresses) {
          for (var i = 0; i < request.addresses.length; i++) {
            var errors = service.validate(request.addresses[i], arrOfValidators, true, i);
            if (!errors) {
              errCount++;
            }
          }
